...
class GenerateRequest(BaseModel):
    ...
    recipient_company: Optional[str] = "Recipient GmbH"
    recipient_address: Optional[str] = "Sample Street 123\n12345 Berlin"
    recipient_name: Optional[str] = ""        # ← NEW: person's name for salutation
    export_format: Optional[str] = "pdf"
    language: Optional[str] = "en"            # ← NEW: "en" or "de"
...
async def generate_quotation(req: GenerateRequest, filename: str):
    ...
    quote = build_quotation(
        bom, costs, req.config,
        req.recipient_company,
        req.recipient_address,
        req.recipient_name or "",      # ← pass name
    )
    ...
    if req.export_format == "excel":
        ...
        data = export_pdf(quote, lang=req.language)    # ← pass language
        return Response(
            content=data,
            media_type="application/pdf",
            headers={"Content-Disposition": f'attachment; filename="quotation_{req.config.quote_number}.pdf"'},
